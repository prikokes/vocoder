...
OUTPUT_JSON = "benchmark_results_repro.json"


HIFIGAN_V1_HPARAMS = dict(
...
MODELS_TO_EVAL = [
    {
        "name":          "hifigan_v1_repro",
        "kind":          "hifigan",
        "hparams":       HIFIGAN_V1_HPARAMS,
        "ckpt_path":     "E:/se/4_course/thesis/audio_vocoder/weights/model_best_hfg_1.pth",
        "sr":            22050,
        "hop_length":    256,
    },
    {
        "name":          "hifigan_v2_repro",
        "kind":          "hifigan",
        "hparams":       HIFIGAN_V2_HPARAMS,
        "ckpt_path":     "E:/se/4_course/thesis/audio_vocoder/weights/model_best_hfg_2.pth",
        "sr":            22050,
        "hop_length":    256,
    },
    {
        "name":          "freev_repro",
        "kind":          "freev",
        "gen_params":    FREEV_GEN_PARAMS,
        "ckpt_path":     "E:/se/4_course/thesis/audio_vocoder/weights/model_best_freev_2.pth",
        "sr":            22050,
        "hop_length":    256,
    },
]
...
# Импортируем нужные ГЕНЕРАТОРЫ (без дискриминаторов)
from src.model.hifigan import Generator as HiFiGANGenerator         # noqa: E402
from src.model.freev import FreeVGenerator                          # noqa: E402

# Если файлы у тебя называются иначе — поправь импорты выше.
# Я предполагаю:
#   src/model/hifigan.py  содержит  class HiFiGAN, class Generator
#   src/model/freev.py    содержит  class FreeV,  class FreeVGenerator


# ============================================================
# Метрики (идентичны твоему скрипту)
# ============================================================
...
# ============================================================
# Обёртка для воспроизведённых моделей
# ============================================================
...
class ReproModelWrapper:
    """
    Обёртка под HiFi-GAN (V1/V2) и FreeV из твоего проекта.
    Создаём напрямую *Generator* (без дискриминаторов), грузим
    в него подсловарь state_dict с префиксом 'generator.'.
    """
    def __init__(self, entry, device):
        self.kind = entry["kind"]
        self.device = device
        self.sr = entry.get("sr", 22050)
        self.hop_length = entry.get("hop_length", 256)
        self.n_mels = 80
        self.n_fft = 1024
        ...
        if self.kind == "hifigan":
            h = HParams(**entry["hparams"])
            self.generator = HiFiGANGenerator(h).to(device)
        elif self.kind == "freev":
            self.generator = FreeVGenerator(**entry["gen_params"]).to(device)
        else:
        ...
        # У HiFiGAN: HiFiGAN.generator = Generator(h)  -> ключи "generator.conv_pre.weight"
        # У FreeV:   FreeV.generator   = FreeVGenerator -> ключи "generator.psp_input_conv.weight"
        ...
        if len(gen_sd) == 0:
            available = sorted(set(k.split('.')[0] for k in full_sd.keys()))
            raise RuntimeError(
                f"Не нашёл ключей с префиксом '{GEN_PREFIX}' в {ckpt_path}. "
                f"Топ-уровневые префиксы: {available}"
            )

        # FreeV-генератор регистрирует буфер 'window', а в чекпоинте его может не быть
        # (если регистрация была в обёрточном классе). На всякий случай strict=False
        # с проверкой.
        result = self.generator.load_state_dict(gen_sd, strict=False)
        missing = list(result.missing_keys)
        unexpected = list(result.unexpected_keys)

        # Разрешаем "missing" только для буферов window/mel_basis_pinv (они и так
        # инициализированы корректно в __init__)
        allowed_missing = {"window", "pimf.mel_basis_pinv"}
        real_missing = [k for k in missing if k not in allowed_missing]
        if real_missing:
            raise RuntimeError(
                f"Missing keys при загрузке {ckpt_path}: {real_missing}"
            )
        if unexpected:
        ...
        if hasattr(self.generator, "remove_weight_norm"):
            try:
                ...
                print(f"  [warn] remove_weight_norm failed: {e}")
    ...
    def __call__(self, audio_gt):
        ...
        # HiFiGAN Generator возвращает [B, 1, T]
        # FreeVGenerator возвращает [B, 1, T] (после audio.unsqueeze(1))
        if audio.dim() == 3:
...
def main():
    ...
    for entry in MODELS_TO_EVAL:
        ...
        print(f"\n=== {name} ===")
        try:
            wrapper = ReproModelWrapper(entry, device=device)
