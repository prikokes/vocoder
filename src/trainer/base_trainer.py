...
class BaseTrainer:
        ...
        for batch_idx, batch in enumerate(
            ...
            if batch_idx % self.log_step == 0:
                self.writer.set_step(
                    (epoch - 1) * self.epoch_len + batch_idx, "train"
                )
                self.logger.debug(
    ...
    def _evaluation_epoch(self, epoch, part, dataloader):
        ...
        with torch.no_grad():
            for batch_idx, batch in tqdm(
                ...
                batch = self.process_batch(
                    batch, metrics=self.evaluation_metrics,
                )

            # set_step с part — writer сам добавит _{part}
            self.writer.set_step(epoch * self.epoch_len, part)

            # БЕЗ префикса {part}_ — writer добавит суффикс сам
            self._log_scalars(self.evaluation_metrics)

            self._log_batch(batch_idx, batch, part)

            if batch is not None and hasattr(self, '_log_audio_samples'):
                self._log_audio_samples(batch, part)
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if (
            ...
            self.logger.warning(
                "Warning: Optimizer or lr_scheduler different. Not resuming."
            )
        else:
